...
class FilamentRenderer {
 public:
  ...
  bool render_ortho_frame(int ortho_idx, const Eigen::Vector2d& pan, double scale, float canvas_w,
                          float canvas_h, uint8_t* out_rgb_buffer, size_t buffer_size);
  ...
  size_t render_batch(const std::vector<Eigen::Vector3d>& positions,
                      ...
                      const std::vector<uint64_t>& timestamps_us, RingBuffer& ring_buffer);
