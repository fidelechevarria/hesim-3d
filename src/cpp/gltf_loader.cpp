...
bool FilamentRenderer::load_scene(const std::string& glb_path) {
    ...
    if (!scene_bounds_.valid || scene_bounds_.radius < 1e-4) {
        ...
        for (size_t i = 0; i < count; ++i) {
            ...
            if (inst) {
                ...
                if (std::abs(eb.halfExtent.x) > 1e-5 || std::abs(eb.halfExtent.y) > 1e-5 || std::abs(eb.halfExtent.z) > 1e-5) {
                    Eigen::Vector3d b_min(eb.center.x - eb.halfExtent.x, eb.center.y - eb.halfExtent.y, eb.center.z - eb.halfExtent.z);
                    Eigen::Vector3d b_max(eb.center.x + eb.halfExtent.x, eb.center.y + eb.halfExtent.y, eb.center.z + eb.halfExtent.z);
        ...
        if (found && (r_max - r_min).norm() > 1e-4) {
            ...
            scene_bounds_.radius = std::max({scene_bounds_.extent.x(), scene_bounds_.extent.y(), scene_bounds_.extent.z()}) * 0.5;
            scene_bounds_.valid = true;
        }
    }
    ...
    std::cout << "[FilamentRenderer] Dynamic Scene Geometry Analyzed:" << std::endl
              << "  - Bounding Box Min:    [" << scene_bounds_.min_point.x() << ", " << scene_bounds_.min_point.y() << ", " << scene_bounds_.min_point.z() << "]" << std::endl
              << "  - Bounding Box Max:    [" << scene_bounds_.max_point.x() << ", " << scene_bounds_.max_point.y() << ", " << scene_bounds_.max_point.z() << "]" << std::endl
              << "  - Center:              [" << scene_bounds_.center.x() << ", " << scene_bounds_.center.y() << ", " << scene_bounds_.center.z() << "]" << std::endl
              << "  - Extent:              [" << scene_bounds_.extent.x() << ", " << scene_bounds_.extent.y() << ", " << scene_bounds_.extent.z() << "]" << std::endl
