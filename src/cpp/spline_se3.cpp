...
void SE3Spline::set_gravity(const Eigen::Vector3d& g) { gravity_ = g; }
...
void SE3Spline::build_from_waypoints(const std::vector<Eigen::Vector3d>& positions,
  ...
  if (positions.size() < 2 || positions.size() != orientations.size()) {
    throw std::invalid_argument(
        "Waypoints must have at least 2 matching positions and orientations.");
  }
  ...
  Eigen::Quaterniond q_ghost_start =
      Eigen::Quaterniond(q0.toRotationMatrix() * exp_so3(-omega_start)).normalized();

  Eigen::Quaterniond q_prev = orientations[N - 2].normalized();
  Eigen::Quaterniond q_end = orientations.back().normalized();
  Eigen::Vector3d omega_end =
      log_so3(q_prev.toRotationMatrix().transpose() * q_end.toRotationMatrix());
  Eigen::Quaterniond q_ghost_end =
      Eigen::Quaterniond(q_end.toRotationMatrix() * exp_so3(omega_end)).normalized();
...
Eigen::Matrix3d SE3Spline::right_jacobian_so3(const Eigen::Vector3d& omega) {
  ...
  if (theta < 1e-6) {
    return Eigen::Matrix3d::Identity() - 0.5 * (Eigen::Matrix3d() << 0, -omega.z(), omega.y(),
                                                omega.z(), 0, -omega.x(), -omega.y(), omega.x(), 0)
                                                   .finished();
  }
  Eigen::Vector3d a = omega / theta;
  Eigen::Matrix3d a_hat;
  a_hat << 0, -a.z(), a.y(), a.z(), 0, -a.x(), -a.y(), a.x(), 0;

  double s = std::sin(theta);
  double c = std::cos(theta);

  return (s / theta) * Eigen::Matrix3d::Identity() + (1.0 - s / theta) * (a * a.transpose()) -
         ((1.0 - c) / theta) * a_hat;
}
...
TrajectorySample SE3Spline::evaluate(double t_sec) const {
  ...
  sample.position =
      b0 * positions_[i] + b1 * positions_[i + 1] + b2 * positions_[i + 2] + b3 * positions_[i + 3];
  sample.linear_velocity = db0 * positions_[i] + db1 * positions_[i + 1] + db2 * positions_[i + 2] +
                           db3 * positions_[i + 3];
  sample.linear_acceleration = d2b0 * positions_[i] + d2b1 * positions_[i + 1] +
                               d2b2 * positions_[i + 2] + d2b3 * positions_[i + 3];
  ...
  Eigen::Vector3d omega_body =
      exp3.transpose() * exp2.transpose() * w1 + exp3.transpose() * w2 + w3;
  sample.angular_velocity_body = omega_body;

  // Ground-truth IMU measurement (specific force in body frame)
  sample.imu_acceleration =
      sample.orientation.conjugate() * (sample.linear_acceleration - gravity_);

  return sample;
}

std::vector<TrajectorySample> SE3Spline::evaluate_batch(
...
SE3Spline SE3Spline::create_circle_lookat(const Eigen::Vector3d& center, double radius,
                                          double height, double duration_sec, double dt_ctrl) {
...
SE3Spline SE3Spline::create_eight_loop(const Eigen::Vector3d& center, const Eigen::Vector3d& extent,
                                       double duration_sec, double speed_factor, double dt_ctrl) {
