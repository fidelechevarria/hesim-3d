...
void GuiApp::render_multi_viewport_grid() {
  ...
  if (ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
    if (active_layout_ == MultiViewLayout::VIEW_4_GRID ||
        active_layout_ == MultiViewLayout::VIEW_2_SPLIT ||
        active_layout_ == MultiViewLayout::VIEW_3_SPLIT) {
      if (std::abs(mouse_pos.x - split_x) < 8.0f && mouse_pos.y >= upper_y &&
          mouse_pos.y <= upper_y + upper_h) {
        s_dragging_col = true;
      }
    }
    if (active_layout_ == MultiViewLayout::VIEW_4_GRID ||
        active_layout_ == MultiViewLayout::VIEW_3_SPLIT) {
      if (std::abs(mouse_pos.y - (upper_y + split_y)) < 8.0f && mouse_pos.x >= 0.0f &&
          mouse_pos.x <= total_w) {
        s_dragging_row = true;
      }
    }
  ...
  } else if ((active_layout_ == MultiViewLayout::VIEW_4_GRID ||
              active_layout_ == MultiViewLayout::VIEW_2_SPLIT ||
              active_layout_ == MultiViewLayout::VIEW_3_SPLIT) &&
             std::abs(mouse_pos.x - split_x) < 8.0f && mouse_pos.y >= upper_y &&
             mouse_pos.y <= upper_y + upper_h) {
    ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeEW);
  }
  ...
  if (s_dragging_row) {
    ...
    ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeNS);
              ...
              active_layout_ == MultiViewLayout::VIEW_3_SPLIT) &&
             std::abs(mouse_pos.y - (upper_y + split_y)) < 8.0f && mouse_pos.x >= 0.0f &&
             mouse_pos.x <= total_w) {
    ImGui::SetMouseCursor(ImGuiMouseCursor_ResizeNS);
  }
  ...
  if (active_layout_ == MultiViewLayout::VIEW_1_SINGLE) {
    ...
    ImGui::SetNextWindowSize(ImVec2(total_w - split_x - gap, upper_h - split_y - gap),
                             ImGuiCond_Always);
  ...
  if (!is_simulating_) {
    ...
    if (active_layout_ == MultiViewLayout::VIEW_4_GRID ||
      ...
      bg->AddLine(ImVec2(split_x, upper_y), ImVec2(split_x, upper_y + upper_h), col_c,
                  s_dragging_col ? 3.0f : 2.0f);
    }
    if (active_layout_ == MultiViewLayout::VIEW_4_GRID ||
        active_layout_ == MultiViewLayout::VIEW_3_SPLIT) {
      ImU32 row_c = s_dragging_row ? IM_COL32(0, 190, 255, 255) : IM_COL32(45, 48, 56, 255);
      bg->AddLine(ImVec2(0, upper_y + split_y), ImVec2(total_w, upper_y + split_y), row_c,
                  s_dragging_row ? 3.0f : 2.0f);
    }
  }
...
void GuiApp::render_single_viewport(int quad_idx, const std::string& name,
  ...
  if (open) {
    ...
    if (current_mode_ == AppMode::TRAJECTORY_STUDIO) {
      available_views = {
          ...
          {ViewportContent::IMU_TELEMETRY,
           ICON_MDI_CHART_BELL_CURVE " IMU Telemetry (Kinematics)"}};
    } else {
      available_views = {
    ...
    if (ImGui::Combo(combo_id.c_str(), &cur_view_idx, view_names.data(),
                     static_cast<int>(view_names.size()))) {
    ...
    if (viewport_views_[quad_idx] == ViewportContent::CAMERA_CLEAN) {
      ...
      if (ImGui::IsItemHovered())
        ImGui::SetTooltip("Reset camera view to initial framing (Shortcut: 'F')");
      ImGui::SameLine();
      if (current_mode_ == AppMode::TRAJECTORY_STUDIO) {
        ImGui::TextColored(ImVec4(0.45f, 0.85f, 1.0f, 1.0f), ICON_MDI_AXIS_ARROW
                           " [Look-Through | Left: Pan | Right: Dolly | Mid: Orbit | Q/E: Roll]");
      } else {
        ImGui::TextColored(ImVec4(0.45f, 0.85f, 1.0f, 1.0f),
                           ICON_MDI_CHECK_CIRCLE_OUTLINE " [Ground Truth Reference (Filament)]");
      }
    } else if (viewport_views_[quad_idx] == ViewportContent::SIMULATED_APS_SENSOR) {
      ImGui::TextColored(ImVec4(1.0f, 0.70f, 0.30f, 1.0f),
                         ICON_MDI_CHIP " [Physical Sensor APS | Blur + Poisson Noise + CFA]");
    } else if (viewport_views_[quad_idx] == ViewportContent::EVS_ACCUMULATION) {
      ImGui::TextColored(ImVec4(0.95f, 0.45f, 0.45f, 1.0f), ICON_MDI_LIGHTNING_BOLT
                         " [EVS Event Slice | Red: ON (+1) | Blue: OFF (-1)]");
    } else if (viewport_views_[quad_idx] == ViewportContent::TOP_ORTHO) {
      ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.35f, 1.0f),
                         ICON_MDI_VIEW_GRID " [Top Ortho | X-Z]");
      ImGui::SameLine();
      ...
      ImGui::TextColored(ImVec4(0.85f, 0.85f, 0.35f, 1.0f),
                         ICON_MDI_VIEW_AGENDA " [Front Ortho | X-Y]");
      ImGui::SameLine();
      ...
      ImGui::TextColored(ImVec4(0.5f, 0.9f, 0.7f, 1.0f), ICON_MDI_CHART_BELL_CURVE
                         " [IMU Telemetry | Gyroscope (rad/s) + Accelerometer (m/s²)]");
    }
    ...
    auto render_aspect_image = [&](uint32_t tex_id, uint32_t tex_w, uint32_t tex_h) {
      if (tex_id != 0 && avail_sz.x > 10 && avail_sz.y > 10) {
        ...
        draw_list->AddLine(ImVec2(cx - 20, cy), ImVec2(cx - 5, cy), IM_COL32(95, 120, 240, 180),
                           1.5f);
        draw_list->AddLine(ImVec2(cx + 5, cy), ImVec2(cx + 20, cy), IM_COL32(95, 120, 240, 180),
                           1.5f);
        draw_list->AddLine(ImVec2(cx, cy - 20), ImVec2(cx, cy - 5), IM_COL32(95, 120, 240, 180),
                           1.5f);
        draw_list->AddLine(ImVec2(cx, cy + 5), ImVec2(cx, cy + 20), IM_COL32(95, 120, 240, 180),
                           1.5f);
    ...
    switch (viewport_views_[quad_idx]) {
      case ViewportContent::CAMERA_CLEAN: {
        ...
        if (avail_sz.x > 10.0f && avail_sz.y > 10.0f) {
          ...
          if (sensor_texture_id_ != 0) {
            ...
            float sensor_aspect = static_cast<float>(sensor_tex_w_) /
                                  std::max(1.0f, static_cast<float>(sensor_tex_h_));
            float avail_aspect = avail_sz.x / avail_sz.y;
            ...
            draw_list->AddRect(ImVec2(frame_x0, frame_y0), ImVec2(frame_x1, frame_y1),
                               frame_border_col, 0.0f, 0, 1.5f);
            ...
            // Top-Left
            draw_list->AddLine(ImVec2(frame_x0, frame_y0), ImVec2(frame_x0 + corner_len, frame_y0),
                               corner_col, 2.5f);
            draw_list->AddLine(ImVec2(frame_x0, frame_y0), ImVec2(frame_x0, frame_y0 + corner_len),
                               corner_col, 2.5f);
            // Top-Right
            draw_list->AddLine(ImVec2(frame_x1, frame_y0), ImVec2(frame_x1 - corner_len, frame_y0),
                               corner_col, 2.5f);
            draw_list->AddLine(ImVec2(frame_x1, frame_y0), ImVec2(frame_x1, frame_y0 + corner_len),
                               corner_col, 2.5f);
            // Bottom-Left
            draw_list->AddLine(ImVec2(frame_x0, frame_y1), ImVec2(frame_x0 + corner_len, frame_y1),
                               corner_col, 2.5f);
            draw_list->AddLine(ImVec2(frame_x0, frame_y1), ImVec2(frame_x0, frame_y1 - corner_len),
                               corner_col, 2.5f);
            // Bottom-Right
            draw_list->AddLine(ImVec2(frame_x1, frame_y1), ImVec2(frame_x1 - corner_len, frame_y1),
                               corner_col, 2.5f);
            draw_list->AddLine(ImVec2(frame_x1, frame_y1), ImVec2(frame_x1, frame_y1 - corner_len),
                               corner_col, 2.5f);

            // 5. Subtle Sensor Info Badge
            char badge_buf[64];
            std::snprintf(badge_buf, sizeof(badge_buf), "%u x %u (Sensor Gate)", sensor_tex_w_,
                          sensor_tex_h_);
      ...
      case ViewportContent::SIMULATED_APS_SENSOR:
        if (!simulation_has_data_) {
          ...
          ImGui::TextColored(ImVec4(1.0f, 0.75f, 0.35f, 1.0f),
                             ICON_MDI_INFORMATION_OUTLINE " Simulation data not baked yet.");
          ...
          if (keyframes_.size() >= 2) {
            if (ImGui::Button(ICON_MDI_ROCKET_LAUNCH " Bake Simulation Now##aps",
                              ImVec2(210, 28))) {
      ...
      case ViewportContent::EVS_ACCUMULATION: {
        if (!simulation_has_data_) {
          ImGui::SetCursorScreenPos(ImVec2(canvas_p0.x + 24.0f, canvas_p0.y + 30.0f));
          ImGui::TextColored(ImVec4(1.0f, 0.75f, 0.35f, 1.0f),
                             ICON_MDI_INFORMATION_OUTLINE " No neuromorphic events generated yet.");
          ImGui::SetCursorScreenPos(ImVec2(canvas_p0.x + 24.0f, canvas_p0.y + 55.0f));
          ImGui::TextDisabled("Microsecond event stream requires physical simulation bake.");
          ImGui::SetCursorScreenPos(ImVec2(canvas_p0.x + 24.0f, canvas_p0.y + 85.0f));
          if (keyframes_.size() >= 2) {
            if (ImGui::Button(ICON_MDI_ROCKET_LAUNCH " Bake Simulation Now##evs",
                              ImVec2(210, 28))) {
              start_simulation_bake();
            }
          } else {
