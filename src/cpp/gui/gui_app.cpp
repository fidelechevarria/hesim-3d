...
void GuiApp::update_simulation_step(double dt) {
    ...
    // Generate dynamic pattern on viewport textures for preview
    ...
    for (uint32_t y = 0; y < sensor_tex_h_; ++y) {
        for (uint32_t x = 0; x < sensor_tex_w_; ++x) {
            ...
            // Simulated checkerboard moving with trajectory
            ...
            // Simulated EVS events along edges
            if (x % 32 == 0 || y % 32 == 0) {
                if (std::sin(phase + x * 0.1) > 0.3) {
                    evs_img_buffer_[idx] = 255; evs_img_buffer_[idx + 1] = 40; evs_img_buffer_[idx + 2] = 40; // Red ON
                } else {
                    evs_img_buffer_[idx] = 40; evs_img_buffer_[idx + 1] = 80; evs_img_buffer_[idx + 2] = 255; // Blue OFF
                }
            } else {
                evs_img_buffer_[idx] = 20; evs_img_buffer_[idx + 1] = 20; evs_img_buffer_[idx + 2] = 20; // Background
            }
        }
    }
    update_gl_textures();
