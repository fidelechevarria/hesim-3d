...
static void gui_signal_handler(int) { g_gui_exit_requested.store(true); }
...
GuiApp::~GuiApp() {
  ...
  if (window_) {
    ...
    if (sensor_texture_id_) {
      glDeleteTextures(1, &sensor_texture_id_);
      sensor_texture_id_ = 0;
    }
    if (evs_texture_id_) {
      glDeleteTextures(1, &evs_texture_id_);
      evs_texture_id_ = 0;
    }
    if (orbit_texture_id_) {
      glDeleteTextures(1, &orbit_texture_id_);
      orbit_texture_id_ = 0;
    }
    if (sim_aps_texture_id_) {
      glDeleteTextures(1, &sim_aps_texture_id_);
      sim_aps_texture_id_ = 0;
    }
    for (int i = 0; i < 3; ++i) {
      if (ortho_texture_id_[i]) {
        glDeleteTextures(1, &ortho_texture_id_[i]);
        ortho_texture_id_[i] = 0;
      }
    }
...
void GuiApp::ensure_data_directories() {
  try {
    ...
    std::filesystem::create_directories(
        (std::filesystem::path(get_user_data_dir()) / "projects").string());
  } catch (const std::exception& e) {
    std::cerr << "[GuiApp] Warning: Failed to create user data directories: " << e.what()
              << std::endl;
  }
}
...
void GuiApp::init_sensor_presets() {
  ...
  available_sensors_.push_back({"alpsentek_eiger", "AlpsenTek Eiger (640x480, 65°)",
                                "AlpsenTek ALVium/Eiger 0.5MP Hybrid Quad-Bayer APS+EVS", 640, 480,
                                65.0, 30.0, 10.0, true, 0.18, 10});

  // 2. iniVation DAVIS346 Hybrid APS+EVS
  available_sensors_.push_back({"davis346", "iniVation DAVIS346 (346x260, 60°)",
                                "iniVation DAVIS346 (346x260 Mono APS+EVS)", 346, 260, 60.0, 30.0,
                                10.0, true, 0.25, 20});

  // 3. Prophesee EVK4 HD Metavision
  available_sensors_.push_back({"prophesee_evk4", "Prophesee EVK4 (1280x720, 70°)",
                                "Prophesee EVK4 HD Metavision Sensor (1280x720 Mono EVS)", 1280,
                                720, 70.0, 0.0, 0.0, false, 0.15, 5});

  // 4. Sony IMX636 HD Event Sensor
  available_sensors_.push_back({"sony_imx636", "Sony IMX636 (1280x720, 70°)",
...
bool GuiApp::switch_active_sensor(const std::string& sensor_id) {
  ...
  if (!chosen) {
    for (const auto& preset : available_sensors_) {
      if (preset.id.find(sensor_id) != std::string::npos ||
          sensor_id.find(preset.id) != std::string::npos) {
        chosen = &preset;
        break;
      }
    }
  ...
  evs_img_buffer_.assign(
      std::max(camera_render_w_ * camera_render_h_, sensor_tex_w_ * sensor_tex_h_) * 3, 20);
  ...
  if (sim_aps_texture_id_ != 0) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, sensor_tex_w_, sensor_tex_h_, 0, GL_RGB,
                 GL_UNSIGNED_BYTE, sim_aps_img_buffer_.data());
  }
  if (evs_texture_id_ != 0) {
    glBindTexture(GL_TEXTURE_2D, evs_texture_id_);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, sensor_tex_w_, sensor_tex_h_, 0, GL_RGB,
                 GL_UNSIGNED_BYTE, evs_img_buffer_.data());
  }
  ...
  if (renderer_) {
    ...
    renderer_->render_frame(sensor_img_buffer_.data(), sensor_img_buffer_.size(),
                            static_cast<uint64_t>(current_time_sec_ * 1e6));
    if (sensor_texture_id_ != 0) {
      glBindTexture(GL_TEXTURE_2D, sensor_texture_id_);
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, camera_render_w_, camera_render_h_, 0, GL_RGB,
                   GL_UNSIGNED_BYTE, sensor_img_buffer_.data());
    }
  }

  std::cout << "[GuiApp] Switched sensor to: " << chosen->id << " (" << sensor_tex_w_ << "x"
            << sensor_tex_h_ << ", FOV=" << sensor_fov_deg_ << " deg, " << sensor_fps_ << " fps)"
            << std::endl;
...
void GuiApp::capture_keyframe_at_current_time() {
  ...
  StudioKeyframe new_kf{current_time_sec_, camera_pos_, rot, q};
  ...
  if (!replaced) {
    ...
    std::sort(
        ...
        [](const StudioKeyframe& a, const StudioKeyframe& b) { return a.time_sec < b.time_sec; });
    for (size_t i = 0; i < keyframes_.size(); ++i) {
...
double GuiApp::timeline_canvas_x_to_time(float mouse_x, float canvas_x0, float canvas_w) const {
  double range = std::max(0.01, timeline_view_t_max_ - timeline_view_t_min_);
  double norm =
      static_cast<double>(mouse_x - (canvas_x0 + 10.0f)) / static_cast<double>(canvas_w - 20.0f);
...
void GuiApp::update_keyframe_pose(int index) {
  if (index >= 0 && index < static_cast<int>(keyframes_.size())) {
    ...
    keyframes_[index].rotation_euler_deg =
        Eigen::Vector3d(camera_yaw_deg_, camera_pitch_deg_, camera_roll_deg_);
    keyframes_[index].orientation =
        euler_deg_to_quat(camera_yaw_deg_, camera_pitch_deg_, camera_roll_deg_);
    rebuild_trajectory();
  }
}
  ...
  for (int i = 0; i <= num_samples; ++i) {
    ...
    if (interp_mode_ == TrajectoryInterpolation::LINEAR_SLERP) {
      for (size_t k = 0; k + 1 < keyframes_.size(); ++k) {
        if (t >= keyframes_[k].time_sec &&
            (t <= keyframes_[k + 1].time_sec || k + 2 == keyframes_.size())) {
          double dt = keyframes_[k + 1].time_sec - keyframes_[k].time_sec;
          double a = (dt > 1e-6) ? std::clamp((t - keyframes_[k].time_sec) / dt, 0.0, 1.0) : 0.0;
          path_samples_.push_back((1.0 - a) * keyframes_[k].position +
                                  a * keyframes_[k + 1].position);
          break;
        }
      }
    } else {
...
bool GuiApp::save_project_to_json(const std::string& path) {
  ...
  f << "    \"exposure_ms\": " << std::fixed << std::setprecision(2) << config_.exposure_ms
    << ",\n";
  f << "    \"accumulation_window_ms\": " << std::fixed << std::setprecision(2)
    << config_.accumulation_window_ms << "\n";
  f << "  },\n";
  f << "  \"trajectory\": {\n";
  f << "    \"duration_sec\": " << std::fixed << std::setprecision(4) << config_.duration_sec
    << ",\n";
  f << "    \"interpolation\": \""
    << (interp_mode_ == TrajectoryInterpolation::SE3_CUMULATIVE_SPLINE ? "spline_se3"
                                                                       : "linear_slerp")
    << "\",\n";
  f << "    \"keyframes\": [\n";
  ...
  for (size_t i = 0; i < keyframes_.size(); ++i) {
    ...
    f << "        \"position\": [" << kf.position.x() << ", " << kf.position.y() << ", "
      << kf.position.z() << "],\n";
    f << "        \"rotation_euler_deg\": [" << kf.rotation_euler_deg.x() << ", "
      << kf.rotation_euler_deg.y() << ", " << kf.rotation_euler_deg.z() << "],\n";
    f << "        \"orientation_xyzw\": [" << kf.orientation.x() << ", " << kf.orientation.y()
      << ", " << kf.orientation.z() << ", " << kf.orientation.w() << "]\n";
...
bool GuiApp::load_project_from_json(const std::string& path) {
  ...
  if (format == "hesim3d_project") {
    ...
    if (!interp.empty()) {
      interp_mode_ = (interp == "linear_slerp") ? TrajectoryInterpolation::LINEAR_SLERP
                                                : TrajectoryInterpolation::SE3_CUMULATIVE_SPLINE;
    }
  }
...
void GuiApp::prompt_save_project_as() {
  ...
  std::string chosen = NativeDialogs::save_file(
      "Save Project As", default_target,
...
void GuiApp::prompt_load_project() {
  ...
  std::string chosen = NativeDialogs::open_file(
      "Open Project or Trajectory", dir,
...
void GuiApp::auto_save_session() {
  ...
  std::string sess_path =
      (std::filesystem::path(get_user_data_dir()) / "last_session.hesim").string();
...
void GuiApp::prompt_save_trajectory_as() {
  ...
  std::string chosen =
      NativeDialogs::save_file("Save Trajectory As", default_target,
...
void GuiApp::prompt_load_trajectory() {
  ...
  std::string chosen = NativeDialogs::open_file(
      "Load Trajectory JSON", dir, {"Trajectory JSON (*.json)", "*.json", "All Files (*.*)", "*"});
...
void GuiApp::prompt_export_dataset_path() {
  ...
  std::string def_file =
      export_modal_h5_path_.empty() ? generate_default_dataset_path() : export_modal_h5_path_;
  std::string default_target;
  try {
    default_target =
        (std::filesystem::path(dir) / std::filesystem::path(def_file).filename()).string();
  } catch (...) {
    default_target = def_file;
  }

  std::string chosen =
      NativeDialogs::save_file("Export Simulated Dataset (HDF5)", default_target,
  ...
  if (!chosen.empty()) {
    ...
    try {
      ...
      export_modal_traj_path_ =
          (p.parent_path() / (p.stem().string() + "_trajectory.json")).string();
    } catch (...) {
    }
  }
}
  ...
  if (interp_mode_ == TrajectoryInterpolation::LINEAR_SLERP) {
    for (size_t i = 0; i + 1 < keyframes_.size(); ++i) {
      if (t >= keyframes_[i].time_sec && t <= keyframes_[i + 1].time_sec) {
        ...
        Eigen::Quaterniond q =
            keyframes_[i].orientation.slerp(alpha, keyframes_[i + 1].orientation);
...
void GuiApp::compute_optimal_initial_camera() {
  Eigen::Vector3d center =
      scene_bounds_.valid ? scene_bounds_.center : Eigen::Vector3d(0.0, 0.0, 0.0);
  ...
  std::cout << "[GuiApp] Automatically framed camera for scene:" << std::endl
            << "  - Scene Center:    [" << center.x() << ", " << center.y() << ", " << center.z()
            << "]" << std::endl
            << "  - Scene Radius:    " << r << std::endl
            << "  - Camera Distance: " << dist << std::endl
            << "  - Initial Eye:     [" << camera_pos_.x() << ", " << camera_pos_.y() << ", "
            << camera_pos_.z() << "]" << std::endl
            << "  - Pitch: " << camera_pitch_deg_ << " deg, Yaw: " << camera_yaw_deg_ << " deg"
            << std::endl;
}
...
void GuiApp::handle_camera_mouse_input(float min_x, float min_y, float max_x, float max_y) {
  ...
  bool is_hovered = (mouse_pos.x >= min_x && mouse_pos.x <= max_x && mouse_pos.y >= min_y &&
                     mouse_pos.y <= max_y);
  ...
  bool is_orbit_down = ImGui::IsMouseDown(ImGuiMouseButton_Middle) ||
...
void GuiApp::update_ortho_texture(int ortho_idx, float canvas_w, float canvas_h) {
  ...
  bool success = renderer_->render_ortho_frame(
      ortho_idx, ortho_pan_[ortho_idx], ortho_scale_[ortho_idx], static_cast<float>(target_w),
      static_cast<float>(target_h), ortho_img_buffers_[ortho_idx].data(),
  ...
  if (success && ortho_texture_id_[ortho_idx] != 0) {
    ...
    if (ortho_tex_w_[ortho_idx] != target_w || ortho_tex_h_[ortho_idx] != target_h) {
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, target_w, target_h, 0, GL_RGB, GL_UNSIGNED_BYTE,
                   ortho_img_buffers_[ortho_idx].data());
      ortho_tex_w_[ortho_idx] = target_w;
      ortho_tex_h_[ortho_idx] = target_h;
    } else {
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, target_w, target_h, GL_RGB, GL_UNSIGNED_BYTE,
                      ortho_img_buffers_[ortho_idx].data());
    }
  }
...
void GuiApp::frame_ortho_view(int ortho_idx) {
  ...
  Eigen::Vector3d frustum_pts[4] = {
      far_center - right_dir * fw + up_dir * fh, far_center + right_dir * fw + up_dir * fh,
      far_center + right_dir * fw - up_dir * fh, far_center - right_dir * fw - up_dir * fh};
...
void GuiApp::handle_ortho_mouse_input(int ortho_idx, float min_x, float min_y, float max_x,
                                      float max_y) {
  if (ortho_idx < 0 || ortho_idx > 2) return;
  ...
  for (size_t i = 0; i < keyframes_.size(); ++i) {
    ...
    float d2 =
        (mouse_pos.x - kp.x) * (mouse_pos.x - kp.x) + (mouse_pos.y - kp.y) * (mouse_pos.y - kp.y);
  ...
  float d2_cam = (mouse_pos.x - cam_screen.x) * (mouse_pos.x - cam_screen.x) +
                 (mouse_pos.y - cam_screen.y) * (mouse_pos.y - cam_screen.y);
  ...
  if (is_hovered &&
      (hovered_ortho_element_[ortho_idx] != -1 || dragging_camera_ortho_idx_ == ortho_idx ||
  ...
  if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && is_hovered) {
    ...
    if (hovered_ortho_element_[ortho_idx] == -2) {
      ...
      is_playing_ = false;
    } else if (hovered_ortho_element_[ortho_idx] >= 0 &&
               static_cast<size_t>(hovered_ortho_element_[ortho_idx]) < keyframes_.size()) {
  ...
  if (dragging_camera_ortho_idx_ == ortho_idx) {
    ...
    if (selected_keyframe_idx_ >= 0 &&
        static_cast<size_t>(selected_keyframe_idx_) < keyframes_.size()) {
  ...
  if (dragging_keyframe_ortho_idx_ == ortho_idx && dragging_keyframe_idx_ >= 0 &&
      static_cast<size_t>(dragging_keyframe_idx_) < keyframes_.size()) {
...
void GuiApp::draw_ortho_map(int ortho_idx, ImDrawList* draw_list, float min_x, float min_y,
  ...
  if (ortho_texture_id_[ortho_idx] != 0 && renderer_) {
    draw_list->AddImage((ImTextureID)(intptr_t)ortho_texture_id_[ortho_idx], ImVec2(min_x, min_y),
                        ImVec2(max_x, max_y));
    ...
    if (ortho_dimming_ > 0.0f) {
      ...
      draw_list->AddRectFilled(ImVec2(min_x, min_y), ImVec2(max_x, max_y),
  ...
  if (r_grid / grid_step > 40.0)
    grid_step *= 5.0f;
  else if (r_grid / grid_step > 20.0)
    grid_step *= 2.0f;
  ...
  for (size_t i = 0; i < keyframes_.size(); ++i) {
    ...
    bool is_kf_dragging = (dragging_keyframe_ortho_idx_ == ortho_idx &&
                           dragging_keyframe_idx_ == static_cast<int>(i));
    float sz = (static_cast<int>(i) == selected_keyframe_idx_ || is_kf_hovered) ? 8.5f : 6.0f;
    ImU32 col = (static_cast<int>(i) == selected_keyframe_idx_) ? IM_COL32(255, 230, 40, 255)
                                                                : IM_COL32(220, 180, 30, 230);
    ...
    draw_list->AddText(ImVec2(kp.x + sz + 3, kp.y - sz), IM_COL32(255, 255, 255, 200),
                       label.c_str());

    if (is_kf_dragging) {
      char kbuf[128];
      std::snprintf(kbuf, sizeof(kbuf), "KF%zu: [%.2f, %.2f, %.2f] m", i + 1,
                    keyframes_[i].position.x(), keyframes_[i].position.y(),
                    keyframes_[i].position.z());
      ImVec2 txt_sz = ImGui::CalcTextSize(kbuf);
      draw_list->AddRectFilled(ImVec2(kp.x + 12, kp.y - 20), ImVec2(kp.x + 18 + txt_sz.x, kp.y - 2),
                               IM_COL32(15, 18, 25, 220), 4.0f);
      draw_list->AddText(ImVec2(kp.x + 15, kp.y - 18), IM_COL32(255, 230, 40, 255), kbuf);
    }
  }

  // Helper lambda to draw dashed line segments
  auto draw_dashed_line = [&](const ImVec2& p0, const ImVec2& p1, ImU32 col, float thickness,
    ...
    for (float d = 0.0f; d < total_dist; d += step) {
      ...
      draw_list->AddLine(ImVec2(p0.x + dir_x * d, p0.y + dir_y * d),
                         ImVec2(p0.x + dir_x * d_end, p0.y + dir_y * d_end), col, thickness);
    }
  };
  ...
  if (ortho_idx == 0)
    view_dir = Eigen::Vector3d(0, -1, 0);  // Top view (looking -Y)
  else if (ortho_idx == 1)
    view_dir = Eigen::Vector3d(0, 0, -1);  // Front view (looking -Z)
  else
    view_dir = Eigen::Vector3d(-1, 0, 0);  // Side view (looking -X)
  ...
  body_faces.push_back({0,
                        {b_apex, b_tl, b_tr},
                        n_top,
                        c_top,
                        c_top.dot(view_dir),
                        false,
                        n_top.dot(view_dir) < -1e-5});
  ...
  body_faces.push_back({1,
                        {b_apex, b_tr, b_br},
                        n_right,
                        c_right,
                        c_right.dot(view_dir),
                        false,
                        n_right.dot(view_dir) < -1e-5});
  ...
  body_faces.push_back({2,
                        {b_apex, b_br, b_bl},
                        n_bot,
                        c_bot,
                        c_bot.dot(view_dir),
                        false,
                        n_bot.dot(view_dir) < -1e-5});
  ...
  body_faces.push_back({3,
                        {b_apex, b_bl, b_tl},
                        n_left,
                        c_left,
                        c_left.dot(view_dir),
                        false,
                        n_left.dot(view_dir) < -1e-5});

  // Back Quad (TL, TR, BR, BL)
  Eigen::Vector3d n_back = -look_dir;
  Eigen::Vector3d c_back = b_base_center;
  body_faces.push_back({4,
                        {b_tl, b_tr, b_br, b_bl},
                        n_back,
                        c_back,
                        c_back.dot(view_dir),
                        true,
                        n_back.dot(view_dir) < -1e-5});
  ...
  std::sort(visible_faces.begin(), visible_faces.end(),
            [](const CameraBodyFace& a, const CameraBodyFace& b) { return a.depth > b.depth; });
  ...
  for (const auto& face : visible_faces) {
    ...
    if (face.is_quad) {
      draw_list->AddQuadFilled(face.pts_2d[0], face.pts_2d[1], face.pts_2d[2], face.pts_2d[3],
                               face_col);
    } else {
  ...
  draw_list->AddCircleFilled(cam_screen, is_cam_hovered ? 5.5f : 4.0f,
                             IM_COL32(255, 255, 255, 255));
  draw_list->AddCircle(cam_screen, is_cam_hovered ? 5.5f : 4.0f, IM_COL32(230, 80, 25, 255), 16,
                       1.8f);

  if (is_cam_dragging) {
    char buf[128];
    std::snprintf(buf, sizeof(buf), "Cam: [%.2f, %.2f, %.2f] m", camera_pos_.x(), camera_pos_.y(),
                  camera_pos_.z());
    ImVec2 txt_sz = ImGui::CalcTextSize(buf);
    draw_list->AddRectFilled(ImVec2(cam_screen.x + 14, cam_screen.y - 20),
                             ImVec2(cam_screen.x + 20 + txt_sz.x, cam_screen.y - 2),
                             IM_COL32(15, 18, 25, 220), 4.0f);
    draw_list->AddText(ImVec2(cam_screen.x + 17, cam_screen.y - 18), IM_COL32(0, 220, 255, 255),
                       buf);
  }
  ...
  if (!config_.scene_path.empty()) {
    try {
      ...
      if (renderer_->load_scene(config_.scene_path)) {
        ...
        // Configure camera clipping planes and intrinsics dynamically based on active sensor FOV
        // and scene scale
        recompute_sensor_optics();
      ...
      std::cerr << "[GuiApp] Warning: Filament renderer init failed (" << e.what()
                << "). Using procedural fallback." << std::endl;
      renderer_.reset();
    }
  }
...
void GuiApp::render_simulation_progress_modal() {
  ...
  if (ImGui::Begin("H-ESIM Scientific Physics Simulation##Modal", nullptr, flags)) {
    ImGui::TextColored(ImVec4(0.35f, 0.85f, 1.0f, 1.0f), ICON_MDI_LIGHTNING_BOLT
                       " Synthesizing Physical Sensor Dynamics (PyTorch CUDA)...");
    ImGui::Separator();
    ImGui::Spacing();

    // Custom dual-contrast clipped high-contrast progress bar
    char prog_overlay[64];
    std::snprintf(prog_overlay, sizeof(prog_overlay), "%.1f%%  [%d / %d f]", sim_progress_ * 100.0f,
                  sim_current_frame_, sim_total_aps_frames_);
    ...
    dl->AddRectFilled(bar_pos, ImVec2(bar_pos.x + bar_w, bar_pos.y + bar_h),
                      IM_COL32(32, 34, 40, 255), rounding);
    dl->AddRect(bar_pos, ImVec2(bar_pos.x + bar_w, bar_pos.y + bar_h), IM_COL32(52, 56, 68, 255),
                rounding);

    // Active fill bar (vibrant gold / amber)
    if (fill_w > 1.0f) {
      dl->AddRectFilled(bar_pos, ImVec2(bar_pos.x + fill_w, bar_pos.y + bar_h),
                        IM_COL32(245, 178, 18, 255), rounding);
    }
    ...
    if (fill_w < bar_w) {
      dl->PushClipRect(ImVec2(bar_pos.x + fill_w, bar_pos.y),
                       ImVec2(bar_pos.x + bar_w, bar_pos.y + bar_h), true);
    ...
    ImGui::TextColored(ImVec4(0.40f, 0.95f, 0.60f, 1.0f), ICON_MDI_CPU_64_BIT " Compute: %s",
                       sim_device_info_.c_str());
    ImGui::TextColored(ImVec4(0.70f, 0.85f, 0.98f, 1.0f), ICON_MDI_ATOM " Noise: %s",
                       sim_noise_model_info_.c_str());

    const char* rate_str = (sim_sampling_preset_ == 0)   ? "300 Hz"
                           : (sim_sampling_preset_ == 1) ? "1000 Hz"
                                                         : "3200 Hz";
    ...
    ImGui::TextColored(ImVec4(0.95f, 0.80f, 0.35f, 1.0f),
                       ICON_MDI_CLOCK_OUTLINE
                       " Elapsed: %02d:%02d | ETA: %02d:%02d | Rate: %s | %zu Events",
...
void GuiApp::render_export_dataset_modal() {
  ...
  if (ImGui::Begin(ICON_MDI_DOWNLOAD " Export Simulated Dataset (HDF5)##ExportModal",
                   &show_export_modal_, flags)) {
    ImGui::TextColored(ImVec4(0.35f, 0.85f, 1.0f, 1.0f),
                       ICON_MDI_DATABASE_EXPORT " Configure Physical Simulation Dataset Export");
    ImGui::Separator();
    ImGui::Spacing();
    ...
    ImGui::Checkbox(ICON_MDI_VECTOR_POLYLINE " Also export companion camera trajectory (.json)",
                    &export_modal_also_traj_);
    if (export_modal_also_traj_) {
      ...
      if (ImGui::Button(ICON_MDI_FOLDER_OPEN " Path...##Traj", ImVec2(100, 0))) {
        std::string chosen = NativeDialogs::save_file(
            "Export Companion Trajectory", export_modal_traj_path_,
            {"Trajectory JSON (*.json)", "*.json", "All Files (*.*)", "*"});
        if (!chosen.empty()) {
    ...
    ImGui::TextColored(ImVec4(0.9f, 0.75f, 0.3f, 1.0f),
                       ICON_MDI_INFORMATION_OUTLINE " Dataset Specifications:");
    ...
    double aps_bytes =
        static_cast<double>(sim_aps_frames_.size()) * (sensor_tex_w_ * sensor_tex_h_ * 3.0);
    ...
    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.8f, 1.0f), "%s",
                       get_clean_scene_name(config_.scene_path).c_str());

    ImGui::Text("Sensor:");
    ImGui::SameLine();
    ImGui::TextColored(ImVec4(0.8f, 0.8f, 0.8f, 1.0f), "%s (%ux%u)", config_.sensor_name.c_str(),
                       sensor_tex_w_, sensor_tex_h_);
    ...
    if (ImGui::Button(ICON_MDI_CHECK " Export Now", ImVec2(btn_w, 28))) {
      if (export_simulated_dataset(export_modal_h5_path_)) {
        export_status_msg_ = "Successfully exported to " +
                             std::filesystem::path(export_modal_h5_path_).filename().string();
...
void GuiApp::trigger_hesim_simulation() { start_simulation_bake(); }

void GuiApp::start_simulation_bake() {
  if (!renderer_ || keyframes_.size() < 2) {
    std::cerr << "[GuiApp] Trajectory requires at least 2 keyframes before running physical sensor "
                 "simulation."
              << std::endl;
    return;
  }
  ...
  if (use_scientific_hesim_) {
    bool ok = init_scientific_bake_bridge(config_.sensor_name, sensor_tex_w_, sensor_tex_h_,
                                          config_.event_threshold, config_.refractory_period_us,
                                          sim_device_info_, sim_noise_model_info_,
                                          recording_output_path_);
    if (!ok) {
      std::cerr
          << "[GuiApp] Warning: Scientific H-ESIM init failed, falling back to C++ approximation."
          << std::endl;
...
void GuiApp::step_simulation_bake() {
  ...
  // =========================================================================
  if (use_scientific_hesim_) {
    for (int step = 0; step < frames_per_tick && sim_current_frame_ < sim_total_aps_frames_;
         ++step) {
      if (sim_cancel_requested_) {
        cancel_simulation_bake();
        return;
      ...
      bool step_ok = step_scientific_bake_bridge(
          sim_batch_render_buf_.data(), sim_batch_render_buf_.size(), sim_sub_timestamps_us_,
          shutter_us, frame_events, blurred_aps_frame, &step_physical_events);

      if (step_ok) {
        sim_total_events_ += step_physical_events;
        // Add display events into sim_events_ uniformly across all frames (never starve frames >
        // 17)
    ...
    sim_progress_ =
        static_cast<float>(sim_current_frame_) / static_cast<float>(sim_total_aps_frames_);
    sim_status_text_ = "PyTorch CUDA: Frame " + std::to_string(sim_current_frame_) + " / " +
                       std::to_string(sim_total_aps_frames_) + " (" +
                       std::to_string(sim_total_events_) + " physical events)...";
  ...
  for (int step = 0; step < frames_per_tick && sim_current_frame_ < sim_total_aps_frames_; ++step) {
    ...
    // Sub-sample exposure integration across the entire frame interval [frame_t, frame_t +
    // sim_dt_aps_]
    for (int s = 0; s < sim_sub_samples_; ++s) {
      double sample_rel_t = ((s + 0.5) / sim_sub_samples_) * sim_dt_aps_;
      double sub_t = frame_t + sample_rel_t;
      ...
      TrajectorySample ts = spline_.evaluate(sub_t);
      renderer_->set_camera_pose(ts.position, ts.orientation);
      renderer_->render_frame(sim_sub_render_buf_.data(), sim_sub_render_buf_.size(),
                              static_cast<uint64_t>(sub_t * 1e6));
              ...
              for (int k = 0; k < n_events; ++k) {
                ...
                if (ev_t - sim_last_event_time_[p_idx] >= refr_sec) {
                  ...
                  if (frame_disp_events < 50000 && sim_events_.size() < MAX_IN_MEMORY_EVENTS) {
                    sim_events_.push_back(
                        {ev_t, static_cast<uint16_t>(x), static_cast<uint16_t>(y), 1});
                    frame_disp_events++;
                  }
  ...
  sim_progress_ =
      static_cast<float>(sim_current_frame_) / static_cast<float>(sim_total_aps_frames_);
  sim_status_text_ = "Baking frame " + std::to_string(sim_current_frame_) + " / " +
                     std::to_string(sim_total_aps_frames_) + " (Motion Blur + EVS)...";
...
void GuiApp::finalize_simulation_bake() {
  ...
  update_simulated_viewport_buffers();
  std::cout << "[GuiApp] Physical sensor simulation bake completed: " << sim_total_frames_
            << " APS frames, " << sim_total_events_ << " events." << std::endl;
}
...
void GuiApp::update_simulated_viewport_buffers() {
  ...
  if (!sim_aps_frames_.empty()) {
    auto it =
        std::lower_bound(sim_aps_frames_.begin(), sim_aps_frames_.end(), current_time_sec_,
                         [](const SimulatedApsFrame& f, double t) { return f.timestamp_sec < t; });
    size_t best_idx = 0;
    ...
    if (best_idx < sim_aps_frames_.size() &&
      ...
      std::memcpy(sim_aps_img_buffer_.data(), sim_aps_frames_[best_idx].rgb_preview.data(),
  ...
  if (!sim_events_.empty()) {
    auto it_begin =
        std::lower_bound(sim_events_.begin(), sim_events_.end(), t_start,
                         [](const SimulatedEvent& ev, double t) { return ev.timestamp_sec < t; });
    auto it_end =
        std::upper_bound(it_begin, sim_events_.end(), t_end,
                         [](double t, const SimulatedEvent& ev) { return t < ev.timestamp_sec; });
  ...
  if (sim_aps_texture_id_ != 0) {
    glBindTexture(GL_TEXTURE_2D, sim_aps_texture_id_);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, sensor_tex_w_, sensor_tex_h_, GL_RGB, GL_UNSIGNED_BYTE,
                    sim_aps_img_buffer_.data());
  }
  if (evs_texture_id_ != 0) {
    glBindTexture(GL_TEXTURE_2D, evs_texture_id_);
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, sensor_tex_w_, sensor_tex_h_, GL_RGB, GL_UNSIGNED_BYTE,
                    evs_img_buffer_.data());
  }
}
...
bool GuiApp::export_simulated_dataset(const std::string& path) {
  ...
  if (!recording_output_path_.empty() && std::filesystem::exists(recording_output_path_)) {
    if (std::filesystem::path(path) != std::filesystem::path(recording_output_path_)) {
      try {
        std::filesystem::copy_file(recording_output_path_, path,
    ...
    std::cout << "[GuiApp] Successfully saved full dataset (" << sim_total_events_ << " events, "
              << sim_aps_frames_.size() << " APS frames) to " << path << std::endl;
    return true;
  }

  bool ok = export_simulation_to_hdf5(path, config_.sensor_name, sim_events_, sim_aps_frames_,
                                      sensor_tex_w_, sensor_tex_h_, spline_, config_.duration_sec);
  if (ok) {
    recording_output_path_ = path;
    try {
      last_dataset_dir_ = std::filesystem::path(path).parent_path().string();
    ...
    std::cout << "[GuiApp] Successfully exported HDF5 dataset (" << sim_total_events_ << " events, "
              << sim_aps_frames_.size() << " APS frames) to " << path << std::endl;
  }
  return ok;
}
    ...
    if (current_mode_ == AppMode::SENSOR_SIMULATION && simulation_has_data_) {
      ...
      for (size_t i = 0; i < camera_render_w_ * camera_render_h_; ++i) {
        ...
        if (diff > config_.event_threshold) {
          evs_img_buffer_[idx] = 255;
          evs_img_buffer_[idx + 1] = 40;
          evs_img_buffer_[idx + 2] = 40;
        } else if (diff < -config_.event_threshold) {
          evs_img_buffer_[idx] = 40;
          evs_img_buffer_[idx + 1] = 80;
          evs_img_buffer_[idx + 2] = 255;
        } else {
