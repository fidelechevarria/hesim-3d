    ...
    if (ImGui::BeginMenuBar()) {
      ...
      if (ImGui::BeginMenu("File")) {
        ...
        if (ImGui::BeginMenu(ICON_MDI_EXPORT " Export")) {
          ...
          if (ImGui::MenuItem(ICON_MDI_DATABASE_EXPORT " Export Simulated Dataset (HDF5)...",
                              nullptr, false, simulation_has_data_)) {
      ...
      if (ImGui::BeginMenu("Edit")) {
        ...
        ImGui::Separator();
        if (ImGui::MenuItem(ICON_MDI_TRASH_CAN_OUTLINE " Delete Selected Keyframe", "Del", false,
                            selected_keyframe_idx_ >= 0)) {
      ...
      if (ImGui::BeginMenu("View")) {
        if (ImGui::BeginMenu(ICON_MDI_VIEW_DASHBOARD " Multi-View Layout")) {
          if (ImGui::MenuItem(ICON_MDI_CHECKBOX_BLANK_OUTLINE " 1 Viewport (Single)", nullptr,
                              active_layout_ == MultiViewLayout::VIEW_1_SINGLE)) {
            set_multi_view_layout(MultiViewLayout::VIEW_1_SINGLE);
          }
          if (ImGui::MenuItem(ICON_MDI_VIEW_SPLIT_VERTICAL " 2 Viewports (Dual Split)", nullptr,
                              active_layout_ == MultiViewLayout::VIEW_2_SPLIT)) {
            set_multi_view_layout(MultiViewLayout::VIEW_2_SPLIT);
          }
          if (ImGui::MenuItem(ICON_MDI_VIEW_AGENDA_OUTLINE " 3 Viewports (Triple Split)", nullptr,
                              active_layout_ == MultiViewLayout::VIEW_3_SPLIT)) {
            set_multi_view_layout(MultiViewLayout::VIEW_3_SPLIT);
          }
          if (ImGui::MenuItem(ICON_MDI_VIEW_GRID " 4 Viewports (2x2 Grid)", nullptr,
      ...
      ImGui::TextColored(ImVec4(0.5f, 0.75f, 1.0f, 1.0f), ICON_MDI_CUBE_OUTLINE " %s",
                         scene_name.c_str());
      ImGui::SameLine();
      ImGui::TextDisabled("|");
      ImGui::SameLine();
      ...
      if (ImGui::BeginCombo("##sensor_preset_combo",
                            (ICON_MDI_CAMERA_IRIS " " + cur_sensor_label).c_str(),
        ...
        for (const auto& preset : available_sensors_) {
          ...
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "%s\nResolution: %ux%u\nFOV: %.1f deg (H)\nAPS FPS: %.1f Hz\nExposure: %.1f ms",
                preset.description.c_str(), preset.width, preset.height, preset.fov_deg,
                preset.aps_fps, preset.exposure_ms);
          }
        }
        ImGui::EndCombo();
      }
      if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "Select active camera & event sensor preset (Updates FOV, resolution, and intrinsics)");
      }
      ...
      if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "Low-Level Sensor Tuning\nContrast Threshold: C = %.2f\nRefractory Dead Time: %d "
            "us\nClick to adjust parameters",
            config_.event_threshold, config_.refractory_period_us);
      }

      if (ImGui::BeginPopup("##SensorTuningPopup")) {
        ImGui::TextColored(ImVec4(0.4f, 0.8f, 1.0f, 1.0f), ICON_MDI_CAMERA_IRIS " %s Tuning",
                           config_.sensor_name.c_str());
        ImGui::Separator();
        ImGui::Spacing();
        ...
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(
              "Log-intensity change threshold for event firing [0.05 - 0.50].\nLower values = "
        ...
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(
              "Pixel refractory dead time in microseconds [1 - 50 us].\nCaps maximum physical "
              "firing rate of individual pixels (1 / t_refr).\nTypical: 3-5 us (modern "
              "Sony/Prophesee), 15-20 us (DAVIS).");
        }
        ...
        ImGui::Checkbox(ICON_MDI_CPU_64_BIT " Scientific H-ESIM (PyTorch / CUDA)",
                        &use_scientific_hesim_);
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(
              "When checked, uses the authentic PyTorch/CUDA engine with calibrated\n6-parameter "
              "CFA beta noise tensors and Gaussian Q-function firing.\nWhen unchecked, uses fast "
              "C++ approximation.");
        }
        ...
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip("Reset C and t_refr to factory defaults for %s",
                            config_.sensor_name.c_str());
        }
      ...
      ImGui::Text(ICON_MDI_CLOCK_OUTLINE " %02d:%05.2fs " ICON_MDI_FILMSTRIP " [%d/%d f]",
                  static_cast<int>(current_time_sec_ / 60.0), std::fmod(current_time_sec_, 60.0),
      ...
      if (current_mode_ == AppMode::TRAJECTORY_STUDIO) {
        ...
        const char* rate_items[] = {ICON_MDI_FLASH " 300 Hz (Fast)", ICON_MDI_COG " 1000 Hz (Std)",
                                    ICON_MDI_CUBE_SCAN " 3200 Hz (HKUST)"};
        ImGui::SetNextItemWidth(145);
        ImGui::Combo("##SimRateCombo", &sim_sampling_preset_, rate_items, 3);
        if (ImGui::IsItemHovered()) {
          ImGui::SetTooltip(
              "Intermediate physical sampling rate for motion blur & microsecond EVS");
        }
        ...
        if (simulation_has_data_ && trajectory_dirty_since_sim_) {
          ...
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "Trajectory keyframes changed since previous bake. Re-baking recommended.");
          }
          ImGui::SameLine();
        }
        ...
        if (font_mono_) ImGui::PushFont(font_mono_);
        ImGui::TextColored(ImVec4(0.4f, 0.9f, 0.55f, 1.0f),
                           ICON_MDI_LIGHTNING_BOLT " %zu Evts  " ICON_MDI_IMAGE_MULTIPLE " %zu F",
                           sim_total_events_, sim_total_frames_);
        if (font_mono_) ImGui::PopFont();
        ...
        if (trajectory_dirty_since_sim_) {
          ...
          if (ImGui::IsItemHovered()) {
            ImGui::SetTooltip(
                "Trajectory modified. Return to Studio to re-bake with new keyframes.");
          }
        }
        ...
        if (ImGui::Button(ICON_MDI_DOWNLOAD " Export (.h5)...", ImVec2(130, 22))) {
          if (export_modal_h5_path_.empty()) {
            export_modal_h5_path_ = recording_output_path_.empty() ? generate_default_dataset_path()
...
void GuiApp::render_timeline_panel() {
  ...
  // Draw clean horizontal splitter divider line (hidden during simulation to avoid overlapping
  // modals)
  if (!is_simulating_) {
    ImDrawList* bg = ImGui::GetBackgroundDrawList();
    ImU32 split_col = s_dragging_timeline ? IM_COL32(0, 190, 255, 255) : IM_COL32(45, 48, 56, 255);
    bg->AddLine(ImVec2(0, panel_y), ImVec2(total_w, panel_y), split_col,
                s_dragging_timeline ? 3.0f : 2.0f);
  }
  ...
  if (ImGui::Begin("##EarthStudioTimeline", nullptr, flags)) {
    ...
    if (current_mode_ == AppMode::TRAJECTORY_STUDIO) {
      ...
      bool can_delete = (selected_keyframe_idx_ >= 0 &&
                         selected_keyframe_idx_ < static_cast<int>(keyframes_.size()));
      ...
      if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
        ImGui::SetTooltip("Delete selected keyframe (Del / Backspace)");
      ImGui::SameLine();
    } else {
      ImGui::TextColored(ImVec4(0.35f, 0.95f, 0.55f, 1.0f),
                         ICON_MDI_PLAY_CIRCLE_OUTLINE " Sensor Inspection Timeline");
      ImGui::SameLine();
    ...
    const char* interp_names[] = {ICON_MDI_VECTOR_CURVE " SE(3) Spline",
                                  ICON_MDI_VECTOR_LINE " Linear + Slerp"};
    ...
    if (ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Current Trajectory File:\n%s\n\nClick to Save As...",
                        current_trajectory_file_.c_str());
    }
    ImGui::SameLine();
    ...
    // Track 2: Rotation
    if (ImGui::TreeNodeEx(ICON_MDI_ROTATE_ORBIT " Camera Rotation",
      ...
      ImGui::SetNextItemWidth(140);
      if (ImGui::DragFloat("Pan (Yaw)", &yaw, 0.5f, -180.0f, 180.0f, "%.1f deg"))
        camera_yaw_deg_ = yaw;

      ImGui::SetNextItemWidth(140);
      if (ImGui::DragFloat("Tilt (Pitch)", &pitch, 0.5f, -89.0f, 89.0f, "%.1f deg"))
        camera_pitch_deg_ = pitch;

      ImGui::SetNextItemWidth(140);
      if (ImGui::DragFloat("Roll", &roll, 0.5f, -180.0f, 180.0f, "%.1f deg"))
        camera_roll_deg_ = roll;
    ...
    ImGui::BeginChild("##TimelineCanvas", ImVec2(right_canvas_w, 0), true,
    ...
    draw_list->AddLine(ImVec2(ov_p0.x, ov_p1.y), ImVec2(ov_p1.x, ov_p1.y),
    ...
    for (const auto& kf : keyframes_) {
      ...
      draw_list->AddLine(ImVec2(ov_kx, ov_p0.y + 2.0f), ImVec2(ov_kx, ov_p1.y - 2.0f),
                         IM_COL32(230, 195, 40, 200), 1.5f);
    }
    ...
    draw_list->AddRectFilled(ImVec2(ov_bx0, ov_p0.y + 1.0f), ImVec2(ov_bx1, ov_p1.y - 1.0f),
                             IM_COL32(0, 150, 215, 60), 2.0f);
    draw_list->AddRect(ImVec2(ov_bx0, ov_p0.y + 1.0f), ImVec2(ov_bx1, ov_p1.y - 1.0f),
                       IM_COL32(0, 190, 255, 200), 2.0f, 0, 1.5f);
    ...
    if (s_dragging_overview) {
      float mouse_norm =
          std::clamp((mouse_pos.x - (ov_p0.x + 10.0f)) / (canvas_sz.x - 20.0f), 0.0f, 1.0f);
    ...
    if (is_canvas_hovered && !s_dragging_overview && dragging_timeline_kf_idx_ < 0) {
      ...
      if (std::abs(wheel) > 0.01f) {
        ...
        double cur_span = timeline_view_t_max_ - timeline_view_t_min_;
        double new_span =
            std::clamp(cur_span * zoom, 0.2, std::max(300.0, config_.duration_sec * 4.0));
    ...
    for (int f = start_f; f <= end_f; f += tick_step) {
      ...
      if (is_major) {
        ...
        if (font_mono_) {
          draw_list->AddText(font_mono_, 12.0f, ImVec2(tx + 3, ruler_p0.y + 2),
                             IM_COL32(190, 195, 205, 255), f_str.c_str());
        } else {
          draw_list->AddText(ImVec2(tx + 2, ruler_p0.y + 2), IM_COL32(180, 180, 180, 255),
                             f_str.c_str());
        }
      }
    }
    ...
    for (int i = 0; i < 7; ++i) {
      ...
      draw_list->AddLine(ImVec2(canvas_p0.x, ly), ImVec2(canvas_p1.x, ly),
                         IM_COL32(40, 44, 52, 180), 1.0f);
    }

    // Guidance message when no or only 1 keyframe exists
    if (keyframes_.empty()) {
      std::string hint = ICON_MDI_INFORMATION_OUTLINE
          " Free Camera Mode | Navigate with mouse and press [K] or [+ Capture] to define "
          "trajectory";
      ...
      std::string hint = ICON_MDI_ALERT_CIRCLE_OUTLINE
          " 1 keyframe set. Move camera and capture at least 1 more keyframe to generate SE(3) "
          "spline.";
      ImVec2 txt_sz = ImGui::CalcTextSize(hint.c_str());
      float tx = canvas_p0.x + std::max(20.0f, (canvas_sz.x - txt_sz.x) * 0.5f);
      float ty = ruler_p1.y + 45.0f;
    ...
    for (size_t k = 0; k < keyframes_.size(); ++k) {
      ...
      if (kx >= canvas_p0.x && kx <= canvas_p1.x) {
        draw_list->AddLine(ImVec2(kx, ruler_p1.y), ImVec2(kx, canvas_p1.y),
                           IM_COL32(90, 85, 40, 120), 1.0f);
      }
      ...
      for (int lane = 0; lane < 6; ++lane) {
        ...
        if (kx >= canvas_p0.x - 10.0f && kx <= canvas_p1.x + 10.0f) {
          ...
          ImU32 dcol = is_selected ? IM_COL32(255, 235, 50, 255)
                                   : (is_hovered ? IM_COL32(255, 220, 90, 255)
                                                 : IM_COL32(220, 190, 40, 230));
    ...
    if (is_canvas_hovered) {
      ...
      } else if (hovered_diamond_kf_idx >= 0 && ImGui::IsMouseClicked(ImGuiMouseButton_Left) &&
                 !io.KeyAlt) {
    ...
    if (dragging_timeline_kf_idx_ >= 0 &&
        dragging_timeline_kf_idx_ < static_cast<int>(keyframes_.size())) {
      if (ImGui::IsMouseDown(ImGuiMouseButton_Left)) {
        ...
        std::sort(keyframes_.begin(), keyframes_.end(),
                  [](const StudioKeyframe& a, const StudioKeyframe& b) {
    ...
    if (dragging_timeline_kf_idx_ < 0 && !s_dragging_overview &&
      ...
      if (in_track_area && ImGui::IsMouseDown(ImGuiMouseButton_Left) && !io.KeyAlt &&
          hovered_diamond_kf_idx < 0) {
    ...
    if (playhead_x >= canvas_p0.x - 10.0f && playhead_x <= canvas_p1.x + 10.0f) {
      draw_list->AddLine(ImVec2(playhead_x, ruler_p0.y), ImVec2(playhead_x, canvas_p1.y),
