...
enum class MultiViewLayout { VIEW_1_SINGLE = 0, VIEW_2_SPLIT, VIEW_3_SPLIT, VIEW_4_GRID };

enum class AppMode { TRAJECTORY_STUDIO = 0, SENSOR_SIMULATION };
...
enum class TrajectoryInterpolation { SE3_CUMULATIVE_SPLINE = 0, LINEAR_SLERP };
...
class GuiApp {
 ...
 private:
  ...
  ViewportContent studio_views_[4]{ViewportContent::TOP_ORTHO, ViewportContent::CAMERA_CLEAN,
                                   ViewportContent::SIDE_ORTHO, ViewportContent::FRONT_ORTHO};

  ViewportContent sim_views_[4]{ViewportContent::CAMERA_CLEAN,
                                ViewportContent::SIMULATED_APS_SENSOR,
                                ViewportContent::EVS_ACCUMULATION, ViewportContent::IMU_TELEMETRY};

  MultiViewLayout active_layout_{MultiViewLayout::VIEW_4_GRID};
  ViewportContent viewport_views_[4]{ViewportContent::TOP_ORTHO, ViewportContent::CAMERA_CLEAN,
                                     ViewportContent::SIDE_ORTHO, ViewportContent::FRONT_ORTHO};
  ...
  int hovered_ortho_element_[3]{
      -1, -1, -1};              // per ortho viewport: -2 = camera, >= 0 = keyframe index, -1 = none
  ...
  void draw_ortho_map(int ortho_idx, ImDrawList* draw_list, float min_x, float min_y, float max_x,
                      float max_y);
